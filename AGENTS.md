...
- **2026-07-28** — **CI gained a check gate.** `deploy.yml` now runs `astro check` and a dependency audit in a job the deploy depends on. `astro check` is the closest thing to a test this site can have without a suite: `translations.ts` is typed `Record<Locale, Translation>`, so a locale missing a key fails the build instead of shipping a blank section. The audit step is deliberately **non-blocking** — the open advisories are Astro SSR paths (server islands, view transitions, spread props) and `sharp`, none of which exist in a prerendered static build; the real fix is the Astro 5→7 jump, tracked in the standards BACKLOG. Supersedes the 2026-07-19 note about there being no checks.
- **2026-07-26** — **The beacon is ON in production.** `deploy.yml` builds with `PUBLIC_NEXO_BEACON_ENABLED=true` and `PUBLIC_NEXO_HUB_URL=https://nexotools.alvarocdev.com`, so the live site emits the pageview beacon (cookieless, honours Do Not Track). Supersedes the 2026-07-24 note that had it off pending the hub's ingest endpoint. **Still open:** the payload sends `location.pathname` only — no query string — so the `?utm_source=…&utm_medium=powered-by` visits coming from the sibling footers still are not distinguishable from direct traffic. Tracked in the standards BACKLOG.
